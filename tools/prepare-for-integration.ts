...
async function updatePackageJsonForIntegration(packageJson: any, microfrontendName: string): Promise<any> {
...
async function createIntegrationChecklist(microfrontendName: string, outputDir: string): Promise<void> {
...
async function prepareForIntegration(config: IntegrationConfig): Promise<void> {
  ...
  const updatedPackageJson = await updatePackageJsonForIntegration(packageJson, config.microfrontendName)
